...
public class ServiceConnection {
    ...
    public void connect(boolean shutdown) {
        ...
        if (shutdown) {
            ...
            if (future.isSuccess()) {
                ...
                Bukkit.getScheduler().runTaskAsynchronously(
                        plugin,
                        () -> Bukkit.getPluginManager().callEvent(new ServiceConnectEvent(cause))
                );
                Bukkit.shutdown();
            }
        } else {
            future.addListener((ChannelFutureListener) f -> {
                if (f.isSuccess()) {
                    ...
                    Bukkit.getPluginManager().callEvent(new ServiceConnectEvent(f.cause()));
    ...
    private void register() {
        ...
        String string = info.saveToJson().toString();
    ...
    public void sendMessage(ServiceMessageInfo info, boolean block) {
        ...
        future.addListener((ChannelFutureListener) f -> {
            if (f.isSuccess()) {
                ...
                Bukkit.getPluginManager().callEvent(new MessageSentEvent(info, f.cause()));
            }
        });
        if (block) {
